    ...
    for (int i = 0; i < size; i++) { //For loop for each player
      ...
      while (players[i].turn) {
        if (players[i].jail == true) { //Checks for jail
          cout << "Player #" << i + 1 << " is in jail. You need to roll a double to escape or wait " << 3 - jailcount << " more turn(s)." << endl;

        }
        cout << "Is player #" << i + 1 << " ready to roll? Type anything to roll." << endl;
        getline(cin, input);
        if (roll(dice1, dice2) == true) { //If the roll function returns true, player rolled a double
          dicedouble = true;
        } else {
          dicedouble = false;
        }
        ...
        if (players[i].jail == true) {
          ...
          if (players[i].token.position[0] == 0) { //This entire if, else if, else statement moves the player's piece
            if (players[i].token.position[1] - (dice1 + dice2) < 0) {
              players[i].token.position[0] = (dice1 + dice2) - players[i].token.position[1];
              players[i].token.position[1] = 0;
            } else {
              players[i].token.position[1] = players[i].token.position[1] - (dice1 + dice2);
            }
          } else if (players[i].token.position[0] == 9) {
            if (players[i].token.position[1] + (dice1 + dice2) > 9) {
              players[i].token.position[0] = 9 - ((dice1 + dice2) - (9 - players[i].token.position[1]));
            ...
            if (players[i].token.position[1] == 9) {
              if (players[i].token.position[0] - (dice1 + dice2) < 0) {
                players[i].token.position[1] = 9 - ((dice1 + dice2) - players[i].token.position[0]);
                players[i].token.position[0] = 0;
              } else {
                players[i].token.position[0] = players[i].token.position[0] - (dice1 + dice2);
              }
            } else {
              if (players[i].token.position[0] + (dice1 + dice2) > 9) {
                players[i].token.position[1] = (dice1 + dice2) - (9 - players[i].token.position[0]);
